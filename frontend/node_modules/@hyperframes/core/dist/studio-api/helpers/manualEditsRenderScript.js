 ...
 */
export function createStudioPositionSeekReapplyScript() {
...
function studioPositionSeekReapplyRuntime() {
    ...
    if (!document.querySelector("[" + PATH_OFFSET_ATTR + '="true"]') &&
    ...
    const reapplyMotionTimeline = () => {
        ...
        for (let i = 0; i < motionEls.length; i++) {
            ...
            try {
                ...
                fromTo.call(tl, el, { ...from }, { ...to, duration, ease: resolvedEase, overwrite: "auto", immediateRender: false }, start);
                applied += 1;
            }
            catch {
                /* malformed JSON — skip */
    ...
    const reapplyAll = () => {
        ...
        for (let i = 0; i < offsetEls.length; i++) {
            ...
            if (x || y) {
                el.style.setProperty("translate", composeTranslate(el, "var(" + OFFSET_X_PROP + ", 0px)", "var(" + OFFSET_Y_PROP + ", 0px)"));
    ...
    const wrapSeekFunctions = () => {
        const a = wrapFn(() => runtimeWindow.__hf?.["seek"], (fn) => {
            if (runtimeWindow.__hf)
                runtimeWindow.__hf["seek"] = fn;
        });
        const b = wrapFn(() => runtimeWindow.__player?.["renderSeek"], (fn) => {
    ...
    wrapSeekFunctions();
    installSeekTrap(runtimeWindow.__hf, "seek", () => runtimeWindow.__hf?.["seek"], (fn) => {
        if (runtimeWindow.__hf)
            runtimeWindow.__hf["seek"] = fn;
    });
    installSeekTrap(runtimeWindow.__player, "renderSeek", () => runtimeWindow.__player?.["renderSeek"], (fn) => {
        if (runtimeWindow.__player)
            runtimeWindow.__player["renderSeek"] = fn;
    });
    let remaining = 120;
