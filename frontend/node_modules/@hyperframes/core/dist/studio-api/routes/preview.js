...
import { createStudioMotionRenderBodyScript, STUDIO_MOTION_PATH, } from "../helpers/studioMotionRenderScript.js";
...
function htmlHasGsap(html) {
    ...
    return (/<script\b[^>]*src=["'][^"']*gsap/i.test(html) ||
...
function htmlHasCustomEase(html) {
    return (/<script\b[^>]*src=["'][^"']*CustomEase/i.test(html) ||
...
function injectStudioMotionScript(html, projectDir, activeCompositionPath) {
    ...
    if (!script)
        return html;
    return injectScriptsIntoHtml(injectStudioMotionDependencies(html, manifestContent), [], [script], false);
}
...
function injectGsapCdnFallback(html) {
    if (html.includes("data-hf-gsap-fallback"))
        return html;
    if (html.includes("<head>"))
        return html.replace("<head>", "<head>" + GSAP_CDN_FALLBACK_SCRIPT);
...
function injectStudioPreviewAugmentations(html, adapter, projectDir, activeCompositionPath) {
    return injectStudioMotionScript(injectGsapCdnFallback(injectProjectSignature(html, resolveProjectSignature(adapter, projectDir))), projectDir, activeCompositionPath);
}
...
export function registerPreviewRoutes(api, adapter) {
    ...
    api.get("/projects/:id/preview", async (c) => {
        ...
        try {
            ...
            bundled = injectStudioPreviewAugmentations(await transformPreviewHtml(bundled, adapter, project, mainCompositionPath), adapter, project.dir, mainCompositionPath);
            return c.html(bundled, 200, previewCacheHeaders(etag));
        }
        catch {
            const main = resolveProjectMainHtml(project.dir, project.id);
            if (main) {
                return c.html(injectStudioPreviewAugmentations(await transformPreviewHtml(main.html, adapter, project, main.compositionPath), adapter, project.dir, main.compositionPath), 200, previewCacheHeaders(etag));
            }
            return c.text("not found", 404);
        }
    });
    ...
    api.get("/projects/:id/preview/comp/*", async (c) => {
        ...
        return c.html(injectStudioPreviewAugmentations(html, adapter, project.dir, compPath), 200, previewCacheHeaders(etag));
    });
