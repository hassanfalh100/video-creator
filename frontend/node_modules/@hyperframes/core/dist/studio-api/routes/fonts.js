...
function collectMacSystemProfilerFonts() {
    ...
    if (!isRecord(parsed) || !Array.isArray(parsed.SPFontsDataType))
        return [];
    const fonts = [];
    for (const fontEntry of parsed.SPFontsDataType) {
        if (!isRecord(fontEntry))
            continue;
        const typefaces = fontEntry.typefaces;
        if (!Array.isArray(typefaces))
            continue;
        for (const typeface of typefaces) {
            if (!isRecord(typeface))
                continue;
...
function listInstalledFontFamilies() {
    ...
    for (const family of collectMacSystemProfilerFonts()) {
        families.add(family);
        if (families.size >= MAX_FONT_RESULTS)
            break;
    }
...
function parseGoogleFontMetadata(value) {
    if (!isRecord(value) || !Array.isArray(value.familyMetadataList))
        return [];
    const families = [];
