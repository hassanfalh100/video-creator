...
function scopeSelector(selector, scope, compositionId, authoredRootId, compoundAuthoredRoot) {
...
export function scopeCssToComposition(css, compositionId, scopeSelectorOverride, authoredRootId, options) {
    ...
    root.walkRules((rule) => {
        if (isInsideGlobalAtRule(rule))
            return;
        rule.selectors = rule.selectors.map((selector) => scopeSelector(selector, scope, trimmedCompositionId, authoredRootId, options?.compoundAuthoredRoot));
    });
