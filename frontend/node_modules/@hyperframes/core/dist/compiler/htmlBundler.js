...
function safePath(projectDir, relativePath) {
    ...
    if (!resolved.startsWith(normalizedBase) && resolved !== resolve(projectDir))
        return null;
    return resolved;
}
...
function shouldInlineAsDataUrl(filePath) {
    const l = filePath.toLowerCase();
...
function maybeInlineRelativeAssetUrl(urlValue, projectDir) {
    ...
    if (!filePath || !shouldInlineAsDataUrl(filePath))
        return null;
...
export async function bundleToSingleHtml(projectDir, options) {
        ...
        for (const host of hosts) {
            ...
            if (innerRoot) {
                ...
                for (const styleEl of [...innerRoot.querySelectorAll("style")]) {
                    ...
                    compStyleChunks.push(compId ? scopeCssToComposition(css, compId, runtimeScope, authoredRootId) : css);
                    styleEl.remove();
                }
                ...
                for (const scriptEl of [...innerRoot.querySelectorAll("script")]) {
                    ...
                    if (externalSrc) {
                        ...
                        compScriptChunks.push(compId
                            ? wrapScopedCompositionScript(scriptEl.textContent || "", compId, "[HyperFrames] composition script error:", runtimeScope, runtimeCompId || compId, authoredRootId)
                ...
                for (const scriptEl of [...innerDoc.querySelectorAll("script")]) {
                    ...
                    if (externalSrc) {
                        ...
                        compScriptChunks.push(compId
                            ? wrapScopedCompositionScript(scriptEl.textContent || "", compId, "[HyperFrames] composition script error:", runtimeScope, runtimeCompId || compId)
                            : wrapInlineScriptWithErrorBoundary(scriptEl.textContent || "", "[HyperFrames] composition script error:"));
                    }
                    scriptEl.remove();
                }
    ...
    if (Object.keys(compVariablesByComp).length > 0) {
        compScriptChunks.unshift(`window.__hfVariablesByComp = Object.assign({}, window.__hfVariablesByComp || {}, ${JSON.stringify(compVariablesByComp)});`);
    }
