...
function selectorTargetsManagedMedia(selector, mediaIndex) {
    ...
    if (!normalized)
        return false;
    if (mediaIndex.hasVideo && /\bvideo\b/i.test(normalized))
        return true;
    if (mediaIndex.hasAudio && /\baudio\b/i.test(normalized))
        return true;
    for (const mediaId of mediaIndex.ids) {
        const escapedId = escapeRegExp(mediaId);
        if (new RegExp(`#${escapedId}(?![\\w-])`).test(normalized) ||
...
function findImperativeMediaControlFindings(ctx) {
    ...
    if (mediaTags.length === 0 || ctx.scripts.length === 0)
        return findings;
