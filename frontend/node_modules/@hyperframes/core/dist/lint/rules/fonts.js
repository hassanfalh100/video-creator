...
export const fontRules = [
    ...
    ({ styles, source }) => {
        ...
        if (googleFontsInLink || googleFontsInImport) {
            findings.push({
                ...
                severity: "warning",
                message: "Composition loads fonts from fonts.googleapis.com. External font requests " +
                    "fail in sandboxed/offline renders and add latency. Use local @font-face " +
                    "declarations with captured .woff2 files instead.",
                fixHint: "Replace the Google Fonts <link> or @import with @font-face { font-family: '...'; " +
    ...
    ({ styles }) => {
        ...
        const undeclared = used.filter((name) => !declared.has(name) && !PRODUCER_BUNDLED_FONTS.has(name));
        if (undeclared.length === 0)
            return findings;
        findings.push({
            code: "font_family_without_font_face",
            severity: "warning",
            message: `Font ${undeclared.length === 1 ? "family" : "families"} used without @font-face declaration: ${undeclared.join(", ")}. ` +
